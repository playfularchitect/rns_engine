...
print("=" * 72)
print("ADD_U64 CANONICAL HARNESS")
print("=" * 72)
print("module :", rns4.__file__)
print("AVX2   :", rns4.HAS_AVX2)
print("M      :", M)
print("has add_u64_io     :", hasattr(rns4, "add_u64_io"))
print("has add_u64_io_omp :", hasattr(rns4, "add_u64_io_omp"))
...
ref = ((x.astype(object) + ADD) % M).astype(np.uint64)

rns4.omp_set_num_threads(2)

y_io = rns4.add_u64_io(x, ADD)
rns4.omp_set_num_threads(1)
y_omp1 = rns4.add_u64_io_omp(x, ADD)
rns4.omp_set_num_threads(2)
y_omp2 = rns4.add_u64_io_omp(x, ADD)
...
for N in sizes:
    ...
    _ = rns4.add_u64_io(warm_x, ADD)
    rns4.omp_set_num_threads(1)
    _ = rns4.add_u64_io_omp(warm_x, ADD)
    rns4.omp_set_num_threads(2)
    _ = rns4.add_u64_io_omp(warm_x, ADD)
    ...
    for run in range(1, RUNS + 1):
        ...
        print(f"  add io   : {(t1-t0)*1000:.2f} ms  ({mio:.1f} M vals/s)")
        print(f"  add omp1 : {(t3-t2)*1000:.2f} ms  ({momp1:.1f} M vals/s)")
        print(f"  add omp2 : {(t5-t4)*1000:.2f} ms  ({momp2:.1f} M vals/s)")
