...
print("=" * 72)
print("MUL_U64 CANONICAL HARNESS")
print("=" * 72)
print("module :", rns4.__file__)
print("AVX2   :", rns4.HAS_AVX2)
print("M      :", M)
print("has mul_u64_io     :", hasattr(rns4, "mul_u64_io"))
print("has mul_u64_io_omp :", hasattr(rns4, "mul_u64_io_omp"))
...
ref = ((x.astype(object) * MULT) % M).astype(np.uint64)

rns4.omp_set_num_threads(2)

y_io   = rns4.mul_u64_io(x, MULT)
rns4.omp_set_num_threads(1)
y_omp1 = rns4.mul_u64_io_omp(x, MULT)
rns4.omp_set_num_threads(2)
y_omp2 = rns4.mul_u64_io_omp(x, MULT)
...
print("auto ok :", np.array_equal(y_auto, ref))
print("all eq  :", np.array_equal(y_io, y_omp1) and np.array_equal(y_io, y_omp2) and np.array_equal(y_io, y_auto))
print()
...
for N in sizes:
    ...
    _ = rns4.mul_u64_io(warm_x, MULT)
    rns4.omp_set_num_threads(1)
    _ = rns4.mul_u64_io_omp(warm_x, MULT)
    rns4.omp_set_num_threads(2)
    _ = rns4.mul_u64_io_omp(warm_x, MULT)
    _ = rns4.mul_u64_auto(warm_x, MULT)
    ...
    for run in range(1, RUNS + 1):
        ...
        print(f"  mul io   : {(t1-t0)*1000:.2f} ms  ({mio:.1f} M vals/s)")
        print(f"  mul omp1 : {(t3-t2)*1000:.2f} ms  ({momp1:.1f} M vals/s)")
        print(f"  mul omp2 : {(t5-t4)*1000:.2f} ms  ({momp2:.1f} M vals/s)")
        print(f"  mul auto : {(t7-t6)*1000:.2f} ms  ({mauto:.1f} M vals/s)")
        print(f"  hash     : {sha16(y_auto)}")
        print()
    ...
    print(f"  auto  mean={mean(auto_m):8.1f} median={med(auto_m):8.1f}")
    print(f"  auto / io   : {med(auto_m)/med(io_m):.2f}x")
    print(f"  auto / omp2 : {med(auto_m)/med(omp2_m):.2f}x")
    print()
